...
function aggregateError(targetError, sourceError) {
  ...
  if (
    sourceError.type &&
    (!targetError.type ||
      Severity[targetError.type] > Severity[sourceError.type])
  ) {
  ...
  if (
    messageFormat !== sourceError.messageFormat &&
    sourceError.messageFormat
  ) {
    messageFormat += " [Caught in: " + sourceError.messageFormat + "]";

    const params =
      sourceError.messageParams !== undefined &&
      sourceError.messageParams !== null
        ? sourceError.messageParams
        : [];
