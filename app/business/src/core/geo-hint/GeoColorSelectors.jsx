...
const selectGlimmer = ({ index = 0, variant = 'default', isLargeArea = false } = {}) => {
  return [
    variant === 'input'
...
const selectIconColor = ({ color, isDisabled = false }) => {
  return [
    !isDisabled && GeoPrivateDefaultColorGeneratedStyles.iconActiveStyles[color],
    isDisabled && GeoPrivateDefaultColorGeneratedStyles.iconDisabledStyles[color],
  ];
};
...
const selectStaticBackgroundColor = ({ isInverted = false, isMuted = false, surface }) => {
...
const selectTextColor = ({ color, isDisabled = false, isInverted = false }) => {
  const colorKey =
    color === 'inverted' ? 'value' : color === 'headingDescription' || color === 'valueLabel' ? 'heading' : color;
  const styles = isInverted
    ? !isDisabled
...
const selectBorderColor = ({ color, isDisabled = false, isFocused = false }) => {
  const colorKey = color === 'selected' ? 'info' : color;
  return [
    !isDisabled && !isFocused && GeoPrivateDefaultColorGeneratedStyles.borderDefaultActiveStyles[colorKey],
    !isDisabled && isFocused && GeoPrivateDefaultColorGeneratedStyles.borderFocusedActiveStyles[colorKey],
    isDisabled && !isFocused && GeoPrivateDefaultColorGeneratedStyles.borderDefaultDisabledStyles[colorKey],
    isDisabled && isFocused && GeoPrivateDefaultColorGeneratedStyles.borderFocusedDisabledStyles[colorKey],
  ];
};

const selectCategoricalBackgroundColor = ({ index = 0 }) => {
  const styles = GeoPrivateDefaultColorGeneratedStyles.categoricalBackgroundIdleStyles;
  return styles[index % Object.keys(styles).length];
};

const selectCategoricalForegroundColor = ({ index = 0 }) => {
  const styles = GeoPrivateDefaultColorGeneratedStyles.categoricalForegroundTextStyles;
  return styles[index % Object.keys(styles).length];
};

const selectInteractiveColorPalette = ({ color, isDisabled = false, isFocused = false, isActive = false }) => {
  return [
    GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundTextWeightStyles[color],
    GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundTextActiveStyles[color],
    GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundIdleStyles[color],
    isFocused && GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundFocusedStyles[color],
    isActive && GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundActiveStyles[color],
    isDisabled && GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundDisabledStyles[color],
    isDisabled && GeoPrivateDefaultColorGeneratedStyles.interactiveBackgroundTextDisabledStyles[color],
  ];
};

const selectInteractiveOverlay = ({ color, isFocused = false, isActive = false }) => {
  return [
    ...
    isFocused && GeoPrivateDefaultColorGeneratedStyles.interactiveOverlayFocusedStyles[color],
    isActive && GeoPrivateDefaultColorGeneratedStyles.interactiveOverlayActiveStyles[color],
  ];
};
...
const selectInteractiveBorder = ({ context, color, isDisabled, isFocused }) => null;
